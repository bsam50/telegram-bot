...
async def lock_cmd(update,ctx):
    if not await can_manage(update,"أدمن"):
        return await reply(update,"❌ الأمر للمشرفين.")
    text=(update.message.text or "").strip()
    parts=text.split(maxsplit=1)
    if len(parts)<2:
        return await reply(update,"استخدم: قفل الروابط أو فتح الروابط")
    action=parts[0]
    raw=parts[1].strip()
    aliases={
        "جمثون":"جمثون","الجمثون":"جمثون","السب":"السب","السب":"السب",
        "الايرانيه":"الإيرانية","الايرانية":"الإيرانية","الكتابة":"الكتابة","الكتابه":"الكتابة",
        "الاباحي":"الإباحي","الاباحي":"الإباحي","تعديل الميديا":"تعديل الميديا","التعديل":"التعديل",
        "الفيديو":"الفيديو","فيديو":"الفيديو","الصور":"الصور","صور":"الصور","الصوره":"الصور",
        "الملصقات":"الملصقات","ملصقات":"الملصقات","المتحركة":"المتحركة","المتحركه":"المتحركة",
        "الدردشة":"الدردشة","الدردشه":"الدردشة","الروابط":"الروابط","روابط":"الروابط","رابط":"الروابط",
        "التاك":"التاك","تاك":"التاك","البوتات":"البوتات","بوتات":"البوتات","المعرفات":"المعرفات","معرفات":"المعرفات",
        "الكلايش":"الكلايش","كلايش":"الكلايش","التكرار":"التكرار","تكرار":"التكرار","التوجيه":"التوجيه","توجيه":"التوجيه",
        "الانلاين":"الانلاين","انلاين":"الانلاين","الجهات":"الجهات","جهات":"الجهات","الدخول":"الدخول","الصوت":"الصوت","صوت":"الصوت",
        "التوجيه بالتقييد":"التوجيه بالتقييد","الروابط بالتقييد":"الروابط بالتقييد","المتحركة بالتقييد":"المتحركة بالتقييد",
        "المتحركه بالتقييد":"المتحركة بالتقييد","الصور بالتقييد":"الصور بالتقييد","الفيديو بالتقييد":"الفيديو بالتقييد",
        "البوتات بالطرد":"البوتات بالطرد"
    }
    # longest aliases first, and normalize Arabic variants
    key=None
    for a,k in sorted(aliases.items(),key=lambda x:-len(x[0])):
        if raw==a:
            key=k; break
    if raw in ("الكل","كل"):
        for k in set(LOCKS.values()): set_mode(update.effective_chat.id,k,"locked" if action=="قفل" else "open")
        return await reply(update,"🔒 تم قفل الكل." if action=="قفل" else "🔓 تم فتح الكل.")
    if key is None:
        return await reply(update,"❌ القفل غير موجود. استخدم م3 لرؤية القائمة.")
    if action=="قفل":
        set_mode(update.effective_chat.id,key,"locked")
    else:
        set_mode(update.effective_chat.id,key,"open")
    return await reply(update,("🔒 تم قفل " if action=="قفل" else "🔓 تم فتح ")+key)

async def feature_cmd(update,ctx):
    if not await can_manage(update,"مدير"):
        return await reply(update,"❌ الأمر للمدراء.")
    p=(update.message.text or "").split(maxsplit=1)
    if len(p)<2:
        return await reply(update,"استخدم: تفعيل الترحيب أو تعطيل الترحيب")
    action,name=p[0],p[1].strip()
    norm={"الاذكار":"الاذكار","الأذكار":"الاذكار","الترحيب":"الترحيب","الحمايه":"الحماية","الحماية":"الحماية","التسليه":"التسلية","التسلية":"التسلية","الخدميه":"الخدمية","الخدمية":"الخدمية","اليوتيوب":"اليوتيوب"}
    name=norm.get(name,name)
    if name not in FEATURES and name not in ("الحماية","التسلية","الخدمية","الترحيب","الردود","اليوتيوب"):
        return await reply(update,"❌ الميزة غير موجودة. استخدم م3 لرؤية القائمة.")
    database.set_setting(update.effective_chat.id,"feature:"+name,"on" if action=="تفعيل" else "off")
    return await reply(update,f"{'✅ تم تفعيل' if action=='تفعيل' else '❌ تم تعطيل'} {name}")
...
async def service_cmd(update,ctx):
    ...
    if text.startswith("تيك "):
        q=text[4:].strip(); return await reply(update,"🎵 رابط تيك توك: "+q)
    if text.startswith("ساوند "):
        q=text[6:].strip(); return await reply(update,"🎧 رابط ساوند: "+q)
...
# ---------- Extended moderation helpers ----------
async def restrict_timed(update,ctx):
    if not await can_manage(update,"أدمن"): return await reply(update,"❌ الأمر للمشرفين.")
    uid=target_id(update)
    if not uid: return await reply(update,"↩️ استخدم الأمر بالرد على الشخص.")
    text=update.message.text or ""
    m=re.search(r"(\d+)\s*(ث|ثانيه|ثانية|د|دقيقه|دقيقة|س|ساعة|ساعه|يوم)",text)
    minutes=60
    if m:
        n=int(m.group(1)); unit=m.group(2)
        minutes=n/60 if unit.startswith('ث') else n if unit.startswith('د') else n*60 if unit.startswith('س') else n*1440
        minutes=max(1,int(minutes))
    until=datetime.now(timezone.utc)+timedelta(minutes=minutes)
    try:
        await ctx.bot.restrict_chat_member(update.effective_chat.id,uid,permissions=ChatPermissions(can_send_messages=False),until_date=until)
        return await reply(update,f"⏱ تم تقييد المستخدم لمدة {int(minutes)} دقيقة.")
    except Exception: return await reply(update,"❌ تعذر التقييد؛ تأكد من صلاحيات البوت.")

async def clear_rank_cmd(update,ctx,label=None):
    if not await can_manage(update,"مدير"): return await reply(update,"❌ الأمر للمدراء.")
    mapping={"المنشئين":"منشئ","المدراء":"مدير","المالكين":"مالك","الادمنيه":"أدمن","المميزين":"مميز"}
    if label is None:
        label=(update.message.text or "").replace("مسح ","",1).strip()
    if label=="الكل": database.clear_rank(update.effective_chat.id)
    elif label in mapping: database.clear_rank(update.effective_chat.id,mapping[label])
    else: return await reply(update,"❌ الرتبة غير معروفة.")
    return await reply(update,"🗑 تم تنفيذ المسح.")

async def list_named_rank(update,ctx,label,rank):
    if not await can_manage(update,"أدمن"): return await reply(update,"❌ الأمر للمشرفين.")
    rows=database.list_rank(update.effective_chat.id,rank)
    if not rows: return await reply(update,f"📋 {label}: لا يوجد")
    lines=[]
    for r in rows:
        lines.append("• "+await user_label(ctx.bot,update.effective_chat.id,r['user_id'])+f" — {r['user_id']}")
    return await reply(update,f"📋 {label}\n━━━━━━━━━━━━\n"+"\n".join(lines))

async def media_lock_enforce(m,c,ctx,key,attr):
    if database.get_lock(c.id,key)=="locked" and getattr(m,attr,None):
        try: await m.delete(); return True
        except: return False
    return False

async def special_admin_cmd(update,ctx):
    if not await can_manage(update,"أدمن"): return await reply(update,"❌ الأمر للمشرفين.")
    t=(update.message.text or "").strip(); c=update.effective_chat.id
    if t in ("طرد البوتات","كشف البوتات"):
        try:
            admins=await ctx.bot.get_chat_administrators(c)
            # Bot API does not expose a full member search; report bots among administrators only.
            bots=[a.user for a in admins if a.user.is_bot]
            if t=="كشف البوتات": return await reply(update,"🤖 البوتات الظاهرة في قائمة المشرفين:\n"+(("\n".join(("@"+u.username if u.username else u.full_name) for u in bots)) if bots else "لا يوجد"))
            for u in bots:
                try: await ctx.bot.ban_chat_member(c,u.id)
                except: pass
            return await reply(update,"🤖 تم طرد البوتات التي استطاع البوت الوصول إليها.")
        except: return await reply(update,"❌ تعذر تنفيذ الأمر.")
    if t in ("منع بالرد","منع") and update.message.reply_to_message:
        uid=update.message.reply_to_message.from_user.id
        database.set_setting(c,f"blocked_user:{uid}","on"); return await reply(update,"🚫 تم منع العضو.")
    if t in ("الغاء منع بالرد","إلغاء منع بالرد") and update.message.reply_to_message:
        uid=update.message.reply_to_message.from_user.id
        database.set_setting(c,f"blocked_user:{uid}","off"); return await reply(update,"✅ تم إلغاء المنع.")
    return None
...
async def enforce(update,ctx):
    m=update.effective_message; c=update.effective_chat; u=update.effective_user
    if not m or not c or c.type not in ("group","supergroup") or not u or u.is_bot: return
    if database.get_setting(c.id,f"blocked_user:{u.id}","off")=="on":
        try: await m.delete()
        except: pass
        return
    try:
    ...
    # Replies
    if database.get_setting(c.id,"feature:الردود","on")=="on":
        r=database.get_reply(c.id,txt.strip())
        if r:
            try: await m.reply_text(r["answer"]); return
            except: pass
    gr=database.get_global_reply(txt.strip())
    if gr:
        try: await m.reply_text(gr["answer"]); return
        except: pass
    def locked(k): return database.get_lock(c.id,k)=="locked"
    try:
        # text/content locks
        if locked("الروابط") and URL_RE.search(txt): await m.delete(); return
        if locked("المعرفات") and ("@" in txt or (m.entities and any(e.type=="text_mention" for e in m.entities))): await m.delete(); return
        if locked("التاك") and ("@" in txt or (m.entities and any(e.type in ("mention","text_mention") for e in m.entities))): await m.delete(); return
        if locked("السب") and any(w in txt.lower() for w in BAD_WORDS): await m.delete(); return
        if locked("الايرانية") and any(w in txt.lower() for w in IRAN_WORDS): await m.delete(); return
        if locked("الإباحي") and any(w in txt.lower() for w in ("porn","xxx","اباحي","إباحي","جنس")): await m.delete(); return
        if locked("الكلايش") and len(txt)>500: await m.delete(); return
        if locked("التوجيه") and m.forward_origin: await m.delete(); return
        if locked("الانلاين") and getattr(m,"via_bot",None): await m.delete(); return
        if locked("الجهات") and getattr(m,"contact",None): await m.delete(); return
        # media locks
        if locked("الصور") and m.photo: await m.delete(); return
        if locked("الفيديو") and m.video: await m.delete(); return
        if locked("المتحركة") and m.animation: await m.delete(); return
        if locked("الملصقات") and m.sticker: await m.delete(); return
        if locked("الصوت") and (m.audio or m.voice): await m.delete(); return
        if locked("التعديل") and getattr(m,"edit_date",None): await m.delete(); return
        if locked("تعديل الميديا") and getattr(m,"edit_date",None) and (m.photo or m.video or m.animation or m.sticker): await m.delete(); return
        # join/bot lock
        if m.new_chat_members:
            for nu in m.new_chat_members:
                if nu.is_bot and (locked("البوتات") or locked("البوتات بالطرد")):
                    try:
                        if locked("البوتات بالطرد"): await ctx.bot.ban_chat_member(c.id,nu.id)
                        await m.delete()
                    except: pass
                    return
        # repeat lock: hash recent same texts per group
        if locked("التكرار") and txt:
            key=f"repeat:{c.id}"
            prev=database.get_setting(c.id,key,"")
            if prev==txt.strip(): await m.delete(); return
            database.set_setting(c.id,key,txt.strip())
        # chat lock means ordinary text messages only
        if locked("الدردشة") and txt: await m.delete(); return
        # restricted variants: mute sender rather than delete
        restricted=[]
        if locked("الروابط بالتقييد") and URL_RE.search(txt): restricted.append("الرابط")
        if locked("التوجيه بالتقييد") and m.forward_origin: restricted.append("التوجيه")
        if locked("المتحركة بالتقييد") and m.animation: restricted.append("المتحركة")
        if locked("الصور بالتقييد") and m.photo: restricted.append("الصور")
        if locked("الفيديو بالتقييد") and m.video: restricted.append("الفيديو")
        if restricted:
            await ctx.bot.restrict_chat_member(c.id,u.id,permissions=ChatPermissions(can_send_messages=False))
            try: await m.reply_text("🔇 تم تقييدك بسبب: "+"، ".join(restricted))
            except: pass
            return
...
async def router(update,ctx):
    ...
    if t in ("الرتب","معرفه الرتب","معرفة الرتب","الرتب في المجموعة"): return await ranks_cmd(update,ctx)
    # Dev commands before generic رفع/مسح
    if await is_dev(update) and (t.startswith(("رفع Dev","تنزيل Dev","اضف رد عام","اضف رد متعدد عام","مسح الرد العام","مسح الردود العامه","حظر عام","كتم عام","ذيع ","تحديث","reload","اعاده تشغيل","إعادة تشغيل")) or t in ("قائمه العام","مسح رتب العام","الاحصائيات","إحصائيات","الردود العامه","الغاء عام","الغاء الحظر العام","مسح المحظورين عام")):
        return await dev_cmd(update,ctx)
    # Timed restriction
    if t.startswith("تقييد ") and re.search(r"\d+\s*(ث|ثانيه|ثانية|د|دقيقه|دقيقة|س|ساعة|ساعه|يوم)",t): return await restrict_timed(update,ctx)
    # Admin rank operations
    ...
    special=await special_admin_cmd(update,ctx) if t in ("طرد البوتات","كشف البوتات","منع بالرد","منع","الغاء منع بالرد","إلغاء منع بالرد") else None
    if special is not None: return special
    # Moderation
    if t.startswith("الغاء الحظر") or t.startswith("إلغاء الحظر"): return await unban_cmd(update,ctx)
    if t.startswith("الغاء الكتم") or t.startswith("إلغاء الكتم") or t.startswith("فك التقييد") or t.startswith("رفع القيود"): return await unmute_cmd(update,ctx)
    if t.startswith("حظر ") or t=="حظر": return await ban_cmd(update,ctx)
    if t.startswith("كتم ") or t=="كتم": return await mute_cmd(update,ctx)
    if t.startswith("طرد ") or t=="طرد": return await kick_cmd(update,ctx)
    if t.startswith("تقييد ") or t=="تقييد": return await mute_cmd(update,ctx)
    # Admin clearing and viewing
    if t.startswith("مسح "):
        clears={"مسح الكل":"الكل","مسح المنشئين":"المنشئين","مسح المدراء":"المدراء","مسح المالكين":"المالكين","مسح الادمنيه":"الادمنيه","مسح المميزين":"المميزين"}
        if t in clears: return await clear_rank_cmd(update,ctx,clears[t])
        if t=="مسح الردود": return await reply_manage(update,ctx)
        if t.startswith("مسح الرد "): return await reply_manage(update,ctx)
        return await delete_cmd(update,ctx)
    # rank list commands
    rank_views={"المالكين":("المالكين","مالك"),"المالكين الاساسيين":("المالكين الاساسيين","مالك أساسي"),"المنشئين":("المنشئين","منشئ"),"الادمنيه":("الادمنيه","أدمن"),"المدراء":("المدراء","مدير"),"المميزين":("المميزين","مميز")}
    if t in rank_views:
        return await list_named_rank(update,ctx,*rank_views[t])
    # Settings
    if t in ("الرابط","القوانين","الحمايه","الحماية","الاعدادت","الإعدادات","المجموعة","المحظورين","انشاء رابط") or t.startswith(("ضع ","اضف رابط ","اضف قناه ","حذف قناه ","تعيين الايدي","مسح الرابط","مسح الترحيب","مسح الايدي")):
        return await settings_cmd(update,ctx)
    # Replies
    if t=="الردود" or t.startswith(("اضف رد ","اضف رد متعدد ","مسح الرد ","مسح الردود")): return await reply_manage(update,ctx)
    # Fun
    ...
    # Services
    if t.startswith(("قوقل ","تطبيق ","تحميل لعبه ","بحث ","تيك ","ساوند ")) or t in ("قران","القرآن","اذكار","شعر","قصائد","اقتباسات","ثريد","قصص","كتب","نادي المطور","من ضافني"):
